...
export function ProjectThumbnail({ project, isPreviewActive = false }: { project: Project; isPreviewActive?: boolean }) {
  ...
  return (
    <div className="relative aspect-[16/10] overflow-hidden bg-slate-100 dark:bg-slate-800" aria-label={preview.alt}>
      ...
      {hasImage ? <img src={preview.image} alt={preview.alt} onError={() => setMediaFailed(true)} className={`absolute inset-0 size-full transition duration-700 ease-out motion-reduce:transform-none ${preview.fit === 'contain' ? `bg-slate-950 object-contain ${isPreviewActive ? 'scale-[1.025]' : 'scale-100'}` : `object-cover ${isPreviewActive ? 'scale-[1.045] translate-x-1' : 'scale-100'}`}`} /> : null}
