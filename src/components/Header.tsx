...
export function Header() {
  const { t } = useLanguage()
  return (
    <header className="sticky top-0 z-30 border-b border-slate-200/80 bg-white/80 backdrop-blur-xl dark:border-slate-700/80 dark:bg-slate-950/80">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between gap-4 px-5 py-4 sm:px-8 lg:px-12">
        <NavLink to="/projects" className="group text-lg font-black tracking-[-0.04em] text-slate-950 dark:text-slate-50">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <nav className="flex items-center gap-1 sm:gap-2" aria-label="Primary navigation">
            {[
              ...
              <NavLink key={item.to} to={item.to} className={({ isActive }) => `rounded-full px-2.5 py-2 text-sm font-bold transition sm:px-4 ${isActive ? 'bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-950' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white'}`}>
                {item.label}
              </NavLink>
            ))}
          </nav>
