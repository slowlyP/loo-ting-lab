...
function ProjectGroup({ title, description, projects }: { title: string; description: string; projects: Project[] }) {
  return (
    <section className="space-y-6">
      <div className="border-b border-slate-200 pb-4 dark:border-slate-700/70">
        <h2 className="text-2xl font-black tracking-[-0.04em] text-slate-950 dark:text-slate-50 sm:text-3xl">{title}</h2>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500 dark:text-slate-400 sm:text-base">{description}</p>
      </div>
...
export function ProjectBrowser() {
  ...
  return (
    <section className="space-y-14 lg:space-y-20">
      <div className="flex items-end justify-between gap-4 border-b border-slate-200 pb-4 dark:border-slate-700/70">
        <h2 className="text-xl font-black tracking-[-0.03em] text-slate-950 dark:text-slate-50">{t('work.gallery')}</h2>
        <p className="text-sm font-semibold text-slate-500 dark:text-slate-400">{projects.length} {t('work.count')}</p>
      </div>
