...
const en: Record<string, ProjectCopy> = {
  ...
  'guild-director': {
    subtitle: 'Mercenary guild management prototype',
    status: 'Early prototype · validating the core loop',
    summary: 'A Unity tactics-management prototype about reviewing tavern candidates, comparing contract terms, and building a mercenary guild.',
    role: 'Building and validating the basic connection between 3×3 auto battles, tavern candidates, contract data, and guild management.',
    highlights: ['3×3 auto-battle basics with frontline blocking, same-column paths, and front-first targeting', 'Separate mercenary definitions and individual instances with unique IDs and names', 'Tavern candidates with personality, stat variance, personal reputation, guild-reputation requirements, and base signing fees'],
    problemSolving: ['Started with small 3×3 placement and targeting rules to validate combat behavior.', 'Separated shared definitions from per-run mercenary instances and connected combat units to those instances.', 'Moved contract conditions into MercenaryCandidate so candidate presentation and contract data have clear boundaries.'],
    detail: {
      overview: ['Project Guild Director is a personal Unity prototype about selecting tavern candidates, reviewing contract terms, and forming a mercenary guild.', 'It is being built step by step around candidate generation, personality and stat variance, personal reputation, required guild reputation, base signing fees, and a 3×3 auto-battle flow.', 'The working title is not final. This is an early prototype, not a released game, and the current focus is data structure and the core loop rather than graphics.'],
      problem: ['The auto-battle rules and mercenary recruitment data need to connect into a repeatable management loop.', 'Shared character definitions must remain separate from per-candidate names, stats, reputation, and contract terms.'],
      coreStructure: ['The 3×3 grid uses melee frontline blocking, same-column attack paths, and front-first target selection as its auto-battle foundation.', 'Mercenary definition data is separate from individual mercenary instances, each with a unique ID and personal name.', 'Combat units reference mercenary instances so roster members can carry into battle.', 'The tavern generates and refreshes candidates with personality, stat variance, and personal reputation.', 'Required guild reputation and base signing fees are stored as contract conditions on MercenaryCandidate.'],
      responsibilities: ['Designing and implementing the combat rules, mercenary data structure, and tavern candidate flow as a personal project.', 'Keeping implemented work, next validation targets, and missing UI or media clearly separated.'],
      implementationStatus: { completed: ['3×3 grid auto-battle foundation', 'Melee frontline blocking and same-column attack paths', 'Front-first target selection', 'Separate mercenary definitions and individual instances', 'Unique IDs and personal names', 'Combat-unit and mercenary-instance connection', 'Tavern candidate generation and refresh', 'Candidate personality and stat variance', 'Personal reputation, required guild reputation, and base signing fees', 'Contract conditions separated into MercenaryCandidate'], inProgress: ['Connecting auto battle and tavern recruitment into one core loop.', 'Validating data structures and game rules before focusing on graphics.'], planned: ['Guild reputation and available funds', 'Available, extra-fee, and refusal contract outcomes', 'Owned-roster updates after successful recruitment', 'Contact book and reconnection flow', 'Actual town and tavern UI'] },
      problemSolving: ['Separated frontline, column, and front-first priorities to verify basic auto-battle targeting.', 'Separated definitions, individual instances, and combat units so recruited mercenaries can carry into battle.', 'Isolated contract values on candidate data so money and reputation checks can be added next.'],
      outcomes: ['Built an initial state where the 3×3 auto battle and tavern candidate flow can be tested.', 'Defined data boundaries for the next contract and owned-roster steps.'],
      scopeLimitations: ['The working title is not final, and this is not a released game.', 'It is an early prototype without final town/tavern UI, completed graphics, images, or video.'],
      milestones: ['Current: validate 3×3 auto battle and tavern candidate/contract data', 'Next: connect guild reputation, funds, and contract outcomes', 'Later: connect the owned roster, contact book, and management UI'],
    },
  },
}
